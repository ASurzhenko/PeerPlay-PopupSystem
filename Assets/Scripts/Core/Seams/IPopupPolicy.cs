...
{
    /// <summary>
    /// Owns the remote kill switch, frequency caps and per-family cooldowns. Consulted twice — once at
    /// submit and again when the request reaches the head — so a switch flipped while the request waited
    /// in the queue still stops it from opening. A refusal is a normal outcome, never an exception.
    /// </summary>
